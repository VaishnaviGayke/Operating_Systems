...
extern int winner;
int 
sys_fork_alternate_winner(void){
  int n;
  if(argint(0, &n) < 0){
    return -1;
  }
...
int sys_tickets_owned(void)
{
	...
	tickets = proc_tickets_owned(pid);
...
int sys_transfer_tickets(void)
{
  ...
  later_tickets = tickets_transfer(pid, tickets, myproc());

	return later_tickets;
}
